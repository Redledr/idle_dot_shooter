...
func calculate_offline_progress(saved_timestamp: float) -> Dictionary:
	...
	var elapsed: float = now - saved_timestamp

	# Cap offline time at 4 hours so early players aren't overwhelmed
	var capped: float = minf(elapsed, 4.0 * 3600.0)

	# Estimate kills per second based on current upgrade state
	var kills_per_second: float = _estimate_kills_per_second()
	var offline_kills: int = int(kills_per_second * capped)
	var offline_currency: float = offline_kills * UpgradeManager.get_dot_value()
...
func _estimate_kills_per_second() -> float:
	...
	var fire_rate: float = UpgradeManager.get_fire_rate()
	var hit_chance: float = 0.70
