...
- `upgrades/CardDatabase.gd`
  - Loads cards from `res://data/cards.json`.
...
- Source of truth for card authoring is `CardDatabase.xlsx`.
- Export script is `python tools/cards_to_json.py`.
- Runtime card loading uses `data/cards.json`.
...
- `main.gd` pauses the tree during card selection, but keeps the HUD and cursor alive with `PROCESS_MODE_ALWAYS`.
